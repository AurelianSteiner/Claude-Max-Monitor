...
### Added
- **Multi-account dashboard**: A single overview showing every Claude and Codex account
  side by side, so switching accounts is no longer needed to see how much quota each one
  has left. Clicking a card makes that account the active one (the menu bar icon follows).
  Cards mark the account that is almost used up and the one with the most headroom.
  Reachable from the menu bar icon (default when a provider has more than one account),
  from the popover's "…" menu, and as a standalone window (right-click the menu bar icon
  or press ⌘D there)
- **Dashboard settings**: New "Overview" card in General Settings — enable/disable the
  overview as the default popover, choose sorting (account order vs. most free first) and
  the number of columns (1–3)
- **Account-scoped API services**: `ClaudeAPIService` / `CodexAPIService` can now be bound
  to a specific account (`init(account:)`) instead of always following the current one.
  Bound instances use isolated cookie storage and write rotated OAuth refresh tokens back
  to their own account, so the parallel per-account fetches cannot cross-contaminate
- **Build without Xcode**: `scripts/build_without_xcode.sh` produces a runnable, ad-hoc
  signed universal `Usage4Claude.app` using only the Command Line Tools
