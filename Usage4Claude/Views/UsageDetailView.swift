...
struct UsageDetailView: View {
    ...
    enum MenuAction {
        ...
        case refreshCodex
        case codexRelogin
        /// Popover auf die Mehrkonten-Übersicht umschalten
        case showDashboard
        /// Popover zurück auf das klassische Einzelkonto-Detail
    ...
    private var refreshAndMenuButtons: some View {
        ...
        ZStack(alignment: .topTrailing) {
            Menu {
                // Bei mehreren Konten zuerst der Weg in die Übersicht
                if UserSettings.shared.canShowDashboard {
                    Button(action: { onMenuAction?(.showDashboard) }) {
                        Label(L.Dashboard.showDashboard, systemImage: "square.grid.2x2")
                    }
