...
final class WebLoginWindowManager {
    ...
    /// Anmeldung im eingebetteten Browserfenster (Cookie-Session).
    ///
    /// Der OAuth-Weg über den Systembrowser braucht einen erreichbaren
    /// localhost-Callback. Klappt der nicht (Firewall, Browser-Variante, Sandbox),
    /// führt dieser Weg zum Ziel: im App-Fenster bei claude.ai anmelden, die App
    /// liest den `sessionKey`-Cookie selbst aus. Kein Redirect, kein Kopieren.
    func showCookieLoginWindow(onAccountCreated: ((Account) -> Void)? = nil) {
        if let window = loginWindow, window.isVisible {
            window.makeKeyAndOrderFront(nil)
            NSApp.activate(ignoringOtherApps: true)
            return
        ...
        let window = makeCompactWindow(title: L.WebLogin.windowTitle, content: loginView, width: 900, height: 700)
