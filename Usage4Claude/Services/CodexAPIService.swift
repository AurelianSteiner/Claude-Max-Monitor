...
class CodexAPIService {
    ...
    ///
    /// 实例按**账户**共享，不是按 Service 实例——当前账户会同时被菜单栏的实例和
    /// Dashboard 的绑定实例拉取，各自持缓存会让同一枚 refresh_token 被并发续期，
    /// 后到者收到 400/401。见 OAuthTokenCacheRegistry。
    private var tokenCache: OAuthTokenCache {
        OAuthTokenCacheRegistry.codex.cache(for: tokenCacheKey)
    }

    private var tokenCacheKey: String {
        (boundAccountId ?? settings.currentCodexAccountId)?.uuidString ?? "codex.unknown"
    }
    ...
    // 默认实例跟随 `settings` 的当前 Codex 账户（菜单栏路径，行为与以前一致）。
    // Dashboard 为每个 Codex 账户单独建一个绑定实例，凭据来自构造时传入的
    // Account，token 缓存 / 进行中任务按实例隔离。
    private let boundAccountId: UUID?
    /// 构造时的凭据快照，仅作为账户已从存储中消失时的兜底
    private let boundSessionTokenFallback: String?

    /// 本实例实际使用的凭据（绑定账户优先，否则取当前 Codex 账户）。
    /// 绑定账户的凭据每次现读，不在实例里缓存——OAuth refresh_token 会在任意一条
    /// 路径续期时轮换，缓存一份就会变陈旧并导致下一轮刷新失败。
    private var activeSessionToken: String {
        guard let id = boundAccountId else { return settings.codexSessionToken }
        return settings.codexAccounts.first { $0.id == id }?.sessionKey
            ?? boundSessionTokenFallback
            ?? ""
    }

    /// 本实例的凭据是否可用
    private var hasActiveCredentials: Bool {
        guard boundAccountId != nil else { return settings.hasValidCodexCredentials }
        return !activeSessionToken.isEmpty
    }

    // MARK: - Initialization

    /// - Parameter account: 绑定的 Codex 账户。传 nil（默认）时跟随当前 Codex 账户。
    init(account: Account? = nil) {
        self.boundAccountId = account?.id
        self.boundSessionTokenFallback = account?.sessionKey
        ...
        if account != nil {
            // Eigener Cookie-Topf je gebundener Instanz — siehe ClaudeAPIService:
            // gleicher Cookie-Name über alle Konten, parallele Anfragen dürfen sich
            // nicht überschreiben, Cloudflare-Cookies müssen aber ablegbar bleiben.
        ...
        return try await withCheckedThrowingContinuation { continuation in
            let task = session.dataTask(with: request) { data, response, error in
                let result: Result<OAuthTokenCache.Tokens, Error> = {
                    ...
                    //
                    // 仅默认实例做这件事：轮换检测读的是全局 HTTPCookieStorage，而绑定
                    // 实例（Dashboard 并行拉取）根本不往那个 Cookie 罐里写，读到的值属于
                    // 当前账户，套到别的账户上就是串号。绑定实例跳过写回，凭据续期仍由
                    // 当前账户的默认实例负责。
    ...
    private func refreshOAuthTokens(refreshToken: String) async throws -> OAuthTokenCache.Tokens {
        ...
        if newRefresh != refreshToken {
            ...
            // 绑定实例写回它自己的账户，避免把 A 账户的新 token 覆盖到当前账户
            let boundId = boundAccountId
            await MainActor.run {
                if let boundId = boundId {
                    UserSettings.shared.silentlyUpdateCodexSessionToken(accountId: boundId, token: newRefresh)
                } else {
    ...
    #if DEBUG
    /// 与 ClaudeAPIService 同理：绑定账户在调试模拟数据上的偏移，
    /// 让 Dashboard 的多张 Codex 卡片呈现不同数值。
    ...
    private func createMockData() -> CodexUsageData {
        ...
        let extraPercentage = mockPercentage(Double(settings.debugCodexExtraUsagePercentage))
        ...
        return CodexUsageData(
            primary: .init(percentage: mockPercentage(Double(settings.debugCodexPrimaryPercentage)), resetsAt: primaryResetAt),
            secondary: .init(percentage: mockPercentage(Double(settings.debugCodexSecondaryPercentage)), resetsAt: secondaryResetAt),
